...
class ItemMods extends Component {
    ...
    matchSortedMod(modifier) {
        if (this.props.mod.pattern.test(modifier)) return { color: '#f73979' };
        return {};
    }
                ...
                <tbody>
                    {this.props.modifiers.implicit != undefined ? this.props.modifiers.implicit.map((element, index) => (
                        <tr key={this.props.id + ' ' + element + index}>
                            <th className='hover' onClick={() => this.getPattern(element, 'implicit')} style={this.matchSortedMod(element)}>{element}</th>
                        </tr>
                    )) : null}
                    {this.props.modifiers.explicit != undefined ? this.props.modifiers.explicit.map((element, index) => (
                        <tr key={this.props.id + ' ' + element + index}>
                            <td className='hover' onClick={() => this.getPattern(element, 'explicit')} style={this.matchSortedMod(element)}>{element}</td>
                        </tr>
                    )) : null}
                    {this.props.modifiers.crafted != undefined ? this.props.modifiers.crafted.map((element, index) => (
                        <tr key={this.props.id + ' ' + element + index}>
                            <td><span style={{ backgroundColor: '#55c3d3', margin: 'auto 0.8rem auto auto', borderRadius: '2px', padding: '3px', color: '#ffffff' }}>Crafted</span> <span className='hover' onClick={() => this.getPattern(element, 'crafted')} style={this.matchSortedMod(element)}>{element}</span></td>
                        </tr>
                    )) : null}
                </tbody>
            </table>
